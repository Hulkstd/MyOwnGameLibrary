...
namespace Game.Threading
{
    public class MonoMultiThread : MonoSingleton<MonoMultiThread>
    {
        private class WorkerThread
        {
            ...
            {
                return workerThread != null && workerThread._key == key;
            }
        ...
        private void Update()
        {
            ...
            lock (LockMainThreadPool)
            {
                ...
                list = list.Where(valueTuple => PauseList.All(key => valueTuple.Key != key)).ToList();
        ...
        {
            var workerThread = new WorkerThread(instance, action, mainThreadAction, initializeData, _workerThreadKey);
        ...
        public static bool ContainThreadWorker(int key)
        {
            lock (LockMainThreadPool)
            {
